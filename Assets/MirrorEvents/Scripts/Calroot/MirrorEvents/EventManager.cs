...
using Calroot.MirrorEvents.Util;
using Mirror;
using UnityEngine;
...
namespace Calroot.MirrorEvents {
	public static class EventManager {
		/// <summary>
		///  This is used to make sure the static constructor is called.
		/// </summary>
		...
		[Server]
		private static void Server_OnNetworkEventReceived(NetworkConnectionToClient connectionToClient,
			ServerNetworkEventMessage eventMessage) {
		...
		///		This may cause race conditions if the listeners of the event on clients relies on SyncVars that are changed by the invoker.
		/// </summary>
		...
		/// <param name="networkEvent"></param>
		[Server]
		public static void Server_InvokeTargetEvent(NetworkConnectionToClient connectionToClient,
		...
		public static void RegisterListeners(object o) {
			foreach (MethodInfo methodInfo in o.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Static |
...
#if UNITY_EDITOR
					Debug.LogError(
						$"Tried to register an event that has more than one NetworkEventHandler attribute! Method: {o.GetType().Name}#{methodInfo.Name}");
#endif
					continue;
				}
...
#if UNITY_EDITOR
					Debug.LogError(
						$"Tried to register a NetworkEventHandler that has an incorrect amount of parameters! Method: {o.GetType().Name}#{methodInfo.Name}, Parameters: {methodInfo.GetParameters().Select(x => x.ParameterType.Name).Stringify()}");
#endif
					continue;
				}
...
#if UNITY_EDITOR
					Debug.LogError(
						$"Tried to register a NetworkEventHandler that has a non-NetworkEvent as a parameter! Method: {o.GetType().Name}#{methodInfo.Name}, Parameter: {type.Name}");
#endif
					continue;
				}
...
#if UNITY_EDITOR
					Debug.LogError(
						$"Tried to register a NetworkEventHandler that doesn't return void! Method: {o.GetType().Name}#{methodInfo.Name}");
#endif
					continue;
				}

				Dictionary<object, List<MethodInfo>> registeredObjectMap =
					listeners.GetValueOrDefault(type, new Dictionary<object, List<MethodInfo>>());
		...
		private static void InvokeEvent(NetworkEvent networkEvent) {
			...
			try {
				...
				if (curr != null && curr.DeclaringType != null) {
					Debug.LogError(
						$"Events cannot be registered or unregistered in event handlers! Method: {curr.DeclaringType.Name}#{curr.Name}");
				}
				...
				if (curr != null && curr.DeclaringType != null) {
					Debug.LogError(
						$"An exception occured in an event handler! Method: {curr.DeclaringType.Name}#{curr.Name}");
				}
